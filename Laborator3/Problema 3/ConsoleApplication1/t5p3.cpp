...
public:
	void arbore(double lungime, int nivel, double factorDiviziune, CPunct &p, CVector v) {
		...
		if (nivel == 0) {
			...
			arbore(lungime, nivel - 1, factorDiviziune, p, v);
			p1 = v.getDest(p, lungime);

			v.rotatie(-45);
			arbore(lungime / 2.0, nivel - 1, factorDiviziune, p1, v);
		}
	}
...
class Patratele {
private:
	void patratele(CPunct p, int nivel, double lungime) {
		if (nivel == 0) {
			...
			double x1, y1;
			p.getxy(x1, y1);
			patratele(CPunct(x1, y1), nivel - 1, lungime / 3);
			patratele(CPunct(x1 + lungime / 3, y1), nivel - 1, lungime / 3);
			patratele(CPunct(x1 + 2 * lungime / 3, y1), nivel - 1, lungime / 3);
			patratele(CPunct(x1, y1 + lungime / 3), nivel - 1, lungime / 3);
			patrat(CPunct(x1 + lungime / 3, y1 + lungime / 3), lungime / 3);

			patratele(CPunct(x1 + 2 * lungime / 3, y1 + lungime / 3), nivel - 1, lungime / 3);
			patratele(CPunct(x1, y1 + 2 * lungime / 3), nivel - 1, lungime / 3);
			patratele(CPunct(x1 + lungime / 3, y1 + 2 * lungime / 3), nivel - 1, lungime / 3);
			patratele(CPunct(x1 + 2 * lungime / 3, y1 + 2 * lungime / 3), nivel - 1, lungime / 3);
		}
	}
	void patrat(CPunct p, double lungime) {
		CVector v(0, -1);
		v.rotatie(90);
		v.deseneaza(p, lungime);
...
public:
	void afisare() {
		patrat(CPunct(-0.5, -0.5), 1);

		patratele(CPunct(-0.5, -0.5), nivel, 1);
	}
};
...
public:
	...
	void curba(double lungime, int nivel, CPunct p, CVector v, int directie)
	{
		if (nivel == 0)
		{
			v.deseneaza(p, lungime);
		...
		{
			v.rotatie(directie * 60);
			curba(lungime / 2, nivel - 1, p, v, -directie);
