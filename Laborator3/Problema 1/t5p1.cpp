...
public:
	Mandelbrot() {
		...
		m.nrIteratii = NRITER_JF;
		m.modMax = MODMAX_JF;
		printf("Constructor fara nici un parametru\n");
	}
	...
	void setnriter(int v) { assert(v <= NRITER_JF); m.nrIteratii = v; }
	int getnriter() { return m.nrIteratii; }

	//testeaza daca x apartine multimill Mandelbrot
	// returneaza 0 daca apartine, -1 daca converge finit, +1 daca converge infinit
	int isIn(CComplex &x,int &numarIteratii) {
		int rez = 0;
		CComplex z0, z1;

		z0 = CComplex(0, 0);
		for (numarIteratii = 1; numarIteratii < m.nrIteratii; numarIteratii++) {
			z1 = z0*z0 + x;
	...
	void display(double xmin, double ymin, double xmax, double ymax) {
		...
		for (double x = xmin; x <= xmax; x += RX_JF) {
			for (double y = ymin; y <= ymax; y += RY_JF) {
				numarIteratii = 0;
				CComplex z(x, y);
				int r = isIn(z,numarIteratii);
				if (r == 0) {
					glColor3f(1.0, 0.1, 0.1);
				}
				else {
					glColor3f(0.033 * numarIteratii, 0.1, 0.1);
				}
